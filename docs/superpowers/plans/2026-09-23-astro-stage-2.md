...
| 22 | Порожній прогрес-бар на `/projects/<slug>/` для проєктів без `progress` у легасі | `load()` після завантаження ставить `hidden=true` на кожен `[data-progress-wrap]`, чий `[data-progress-bar]` так і не отримав inline `width`; порівнюються по пікселях без масок і без `known` | легасі-розмітка `project.dc.html` містить `<div class="info-progress" data-progress-wrap hidden>`, але компіляція шаблону `<x-dc>` губить бере-атрибут `hidden` — блок лишається в потоці документа (`display:block`), а порожній `<span data-progress-bar>` без inline-`width` займає 100% ширини `.bar`, рендерячись суцільною смугою `var(--accent)`. Легасі-скрипт лише вмикає `hidden=false`, коли `progress` є, і ніколи не вимикає — сама розмітка мала прийти вже схованою. Нова сторінка такий блок для проєкту без `progress` не рендерить узагалі, тож порівняння відновлює задуманий легасі-стан, а не ховає різницю: `scripts/visual/run.mjs`, `load()` |
| 23 | Стрічка Facebook у `#media` головної вантажиться недетерміновано | `scripts/visual/run.mjs` одразу після `newContext` блокує запити до `facebook.(net\|com)` (`context.route(...).abort()`) для обох збірок; маска `.fb-page` лишається; порівнюються по пікселях без `known` | це не відмінність легасі від нової збірки, а недетермінованість стороннього вмісту: кожна збірка незалежно отримує то iframe на 362px, то запасний блок на 66–88px. Маска ховає пікселі, але не висоту, тож `#media` падає за розміром, а все нижче зсувається. Коли Facebook заблоковано з обох боків, обидві сторінки показують той самий статичний запасний блок |
