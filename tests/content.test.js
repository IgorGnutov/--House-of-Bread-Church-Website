...
test('адреса продубльована на головній: контакти й підвал показують різний текст', () => {
  ...
  // Знахідка 6: у легасі-головній (index.html, видалена на Етапі 2) con.addr
  // траплявся двічі з РІЗНИМ українським текстом: у секції контактів (з
  // друкарською помилкою «Караманиць») і в підвалі (без неї). Скрипт
  // екстракції брав перше входження (контакти) для contacts.addr, тому
  // підвальний варіант заведений окремим полем footer.addr — обидва лишились
  // такими й після видалення джерела.
  assert.equal(
...
test('дев’ять сторінок мають запис: шість перенесених і три нові', () => {
  ...
  for (const slug of ['about', 'contacts', 'donate']) {
    // Знахідка 1: body — робота замовника, null означає «ще немає». Але
    // сам факт null тут навмисно не перевіряємо — щойно замовник впише
    // прозу, цей тест не має падати (isPageEnabled/pages-toggle.test.js
    // перевіряють поведінку build/футера для обох станів).
...
test('ctaUrl проєкту — зовнішня адреса або шлях сайту', () => {
  ...
  // Дірка 11: посилання у стилі легасі index.html#contacts на новій адресації
  // вело б у нікуди — на Етапі 2 такого маршруту вже не існує.
